...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const jadwalTable = {!! json_encode($jadwalTable) !!}; // Data jadwal IRS
    ...
    // Tampilkan jadwal IRS
    jadwalTable.forEach(jadwal => {
        ...
        if (row) {
            ...
            if (cell) {
                ...
                jadwalInfo.classList.add('p-2', 'mt-2', 'mb-0', 'w-full', 'bg-amber-100/80', 'rounded-lg');
                ...
                jadwalInfo.innerHTML = `
                    ...
                    </div><div class="flex justify-center gap-1" style="font-size: 10px;">
                ...
                `;
                cell.appendChild(jadwalInfo);
                const batalkanButton = document.createElement('button');
                batalkanButton.classList.add('bg-amber-400', 'text-white', 'px-4', 'py-1', 'rounded-b-lg', 'w-full', 'mt-0');
                batalkanButton.style.fontSize = '13px';
                batalkanButton.textContent = 'Batalkan';

                // Periksa apakah tombol sudah ada (untuk menghindari duplikasi)
                if (!cell.querySelector('.batalkan-button')) {
                    batalkanButton.classList.add('batalkan-button');
                    cell.appendChild(batalkanButton);
                }

                // Event listener untuk tombol Batalkan
                batalkanButton.addEventListener('click', function() {
                    const jadwalid = jadwal.jadwalid;
                    const nim = '{{ $mahasiswa->nim }}'; // Pastikan ini tersedia
                    const smt = '{{ $mahasiswa->smt }}'; // Pastikan ini tersedia
                    ...
                    // Kirim AJAX untuk batalkan jadwal
                    $.ajax({
                        url: '/batalkan-jadwal', // URL untuk menghapus jadwal
                        method: 'POST',
                        data: {
                            nim: nim,
                            jadwalid: jadwalid,
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                // Hapus jadwal dari tampilan
                                cell.removeChild(jadwalInfo);
                                // Ganti teks tombol menjadi "Batalkan" jika masih ada
                                batalkanButton.textContent = 'Batalkan';
                            } else {
    ...
    // Setelah menampilkan jadwal IRS, tampilkan jadwal dengan kode_mk yang sama
    jadwalLain.forEach(jadwal => {
        ...
        if (row) {
            ...
            if (cell) {
                const jadwalInfo = document.createElement('div');
                jadwalInfo.classList.add('p-2', 'mt-2', 'mb-0', 'w-full', 'bg-red-100/80', 'rounded-t-lg', 'opacity-50', 'text-red-700');
                jadwalInfo.style.fontSize = '13px';
                jadwalInfo.innerHTML = `
                    <p class="text-center font-semibold">${jadwal.nama}</p>
                    <div class="flex justify-center gap-1" style="font-size: 10px;">
                ...
                const batalkanButton = document.createElement('button');
                batalkanButton.classList.add('bg-red-500/50', 'text-white', 'px-4', 'py-1', 'rounded-b-lg', 'w-full', 'mt-0');
                batalkanButton.style.fontSize = '13px';
                batalkanButton.textContent = 'Tidak dapat dipilih';
                ...
                // Event listener untuk tombol Batalkan
                batalkanButton.addEventListener('click', function() {
                    alert('Anda sudah memilih mata kuliah ini pada jadwal lain.');
                });
            }
        }
    });
